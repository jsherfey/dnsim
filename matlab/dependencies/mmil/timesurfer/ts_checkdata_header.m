...
if isstruct(data) && issubfield(data,'parms.filename')
  ...
  for f = 1:length(hdr.parms.filename)
    ...
    mmil_logstr(parms,'%s: scanning file: %s',mfilename,hdr.parms.filename{f});    
    ...
    try
      ...
      n = n + 1;
      clear keep
      mmil_logstr(parms,'%s: found %s',mfilename,s);
    catch
      rmfiles = [rmfiles f];
      mmil_logstr(parms,'%s: failed to load %s',mfilename,hdr.parms.filename{f});
    end
    clear S s
  end
...
if ~strcmp(parms.precision,'skip'), checkprecision; end
if ~exist('data','var')
  mmil_logstr(parms,'%s: no data found.',mfilename);
  data = [];
end
    ...
    for p = 1:length(dataparam)
      for cc = 1:length(data.(datafield))
        if ~strcmp(class(data.(datafield)(cc).(dataparam{p})),parms.precision)
          mmil_logstr(parms,'%s: converting data to %s precision.',mfilename,parms.precision);
