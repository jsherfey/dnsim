...
%Initialize lstmessage for holding exception messages
lstmessage = [];
...
switch o
  ...
  case {'display','preview'}
    ...
    try
      ...
      switch n
        case 0
          error(message('database:vqb:dataDisplayError'))
        case 1
          xchrt(plotfun,xdata);
        case 2
          xchrt(plotfun,ydata);
...
if ~agg
  ...
  %Aggregate column data (will really only make sense for single column)
  strflag  = ischar(x{1});
...
function errchrt(lstmessage)
%ERRCHRT Chart usage table.
%   ERRCHRT displays the error for last charting operation.

cla
u = uicontrol('Style','text','String',lstmessage,'Fontweight','bold',...
