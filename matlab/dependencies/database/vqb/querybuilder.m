...
%   QUERYBUILDER will be removed in a future release. Use DEXPLORE instead.

%   Copyright 1984-2012 The MathWorks, Inc.

%Deprecation message
newFuncString = 'DEXPLORE';

if feature('hotlinks') && ~isdeployed
    %Create DEXPLORE doc link
    openTag = sprintf('<a href = "matlab: helpview([docroot ''/toolbox/database/database.map''],''dexplore_migration'');">');
    closeTag = '</a>';
    newFuncString = [openTag newFuncString closeTag];
end

warning(message('database:vqb:FunctionToBeRemoved', newFuncString))

%Check for Database Toolbox license
if ~(license('test','database_toolbox'))
  errordlg('Database Toolbox not licensed.  Cannot open Visual Query Builder.')
  return
end
...
%GUI spacing
[~,~,bspc,bhgt,bwid,bwid2] = spaceparams([],[],[]);

%Return valid data sources
[datasources,jdbcinfo] = getdatasources; 
if ~iscell(jdbcinfo) && ~isempty(jdbcinfo) && (jdbcinfo == -1)
  return
end

if isempty(datasources)  %Empty datasources indicates no defined sources
  %User will define datasources by setting JDBCDataSourceFile with
  %setdbprefs
...
uipanel('Units','pixels','bordertype','beveledout',...
...
ui.sources = uicontrol('Style','listbox','String',datasources,'Callback',{@sources,f},...
  ...
  'Max',2,'Value',[],'Userdata',jdbcinfo,...
  'Position',[2*bspc top-(2*bspc+6*bhgt) 1.5*bwid 4*bhgt-2*bspc]);

%Data source catalog/schema uicontrols
uicontrol('Style','text','String','Catalog',...
   'Position',[3*bspc+1.5*bwid top-(4*bspc+2.5*bhgt) bwid bhgt]);
ui.catalogs = uicontrol('Style','popupmenu','String','<default>','Callback',{@catalogs,f},...
  'Tag','catalogs','Tooltip','Catalogs in selected data source','Value',1,...
  'Position',[3*bspc+1.5*bwid top-(0.5*bspc+4*bhgt) 1.5*bwid-bspc bhgt]);
uicontrol('Style','text','String','Schema',...
   'Position',[3*bspc+1.5*bwid top-(2.5*bspc+5*bhgt) bwid bhgt]);
ui.schemas = uicontrol('Style','popupmenu','String','<default>','Callback',{@schemas,f},...
  'Tag','schemas','Tooltip','Schemas in selected data source','Value',1,...
  'Position',[3*bspc+1.5*bwid top-(1.5*bspc+6*bhgt) 1.5*bwid-bspc bhgt]);

%Data Source table uicontrols
uicontrol('Style','text','String','Tables',...
   'Position',[3*bspc+3*bwid top-(4*bspc+2*bhgt) bwid bhgt]);
ui.tables = uicontrol('Style','listbox','String','','Max',2,...
   'Tag','tables','Callback',{@tablesvqb,f},...
   'Tooltip','Tables in selected data source and schema if applicable',...
   'Position',[3*bspc+3*bwid top-(2*bspc+6*bhgt) 1.5*bwid 4*bhgt-2*bspc]);

%Table fieldnames
uicontrol('Style','text','String','Fields',...
   'Position',[4*bspc+4.5*bwid top-(4*bspc+2*bhgt) bwid bhgt]);
ui.fields = uicontrol('Style','listbox','String','',...
   'Tag','fields','Max',2,'Callback',{@buildquery,f},... 
   'Tooltip','Fields in selected table',...
   'Position',[4*bspc+4.5*bwid top-(2*bspc+6*bhgt) 1.5*bwid 4*bhgt-2*bspc]);
...
uimenu(qm,'Label','Define &JDBC Data Source...','Separator','on',...
  'Callback',{@definejdbc,f});
if ispc
  uimenu(qm,'Label','Define &ODBC Data Source...','Callback',{@defineodbc,f});
end
uimenu(qm,'Label','&Preferences...','Tag','Preferences',...
  'Callback','preferences(''Database Toolbox'')');
uimenu(qm,'Label','&Generate MATLAB file...','Tag','genmfile',...
  'Callback',{@genmfile,f});

%Exit choice
uimenu(qm,'Label','&Close Visual Query Builder','Tag','Exit','Callback',{@exitvqb,f},'Separator','on');
...
if any(strcmp({'PCWIN','PCWIN64','SOL2'},computer))
  repenable = 'on';
else
  repenable = 'off';
end

if (exist('rptgen','dir') && exist('report','file')) %check for report generator
  repgenenable = 'on';
  repenable = 'on';
else
  repgenenable = 'off';
...
function acceptsubquery(obj,evd,frame)  %#ok
...
setappdata(dobj,'SUBQUERYDATA',wcdat)
setappdata(dobj,'SUBQUERYSTRING',wcstr) 
setappdata(dobj,'TABLEVALUES',get(ui.stables,'Value'));
setappdata(dobj,'FIELDVALUES',get(ui.sfields,'Value'));
...
function andor(obj,evd,frame) %#ok
...
function [tn,tt] = alttables(c,ctg,p,d,sch)
...
%   D is the connection meta data.  S is the schema name.
...
switch upper(p(1:3))
  case 'ORA'
    ...
    if ~exist('nt','var') && ~exist('vt','var')
      set(f,'Pointer','arrow')
      error(message('database:vqb:noTableInfo'))
    end
    if ~exist('nt','var'),nt = [];nn = [];end
    if ~exist('vt','var'),vt = [];vn = [];end
    tn = [nn;vn];
	tt = [nt;vt];
  otherwise
	tinfo = tables(d,ctg,sch);
    if ~isempty(tinfo)
      tn = tinfo(:,1);
      tt = tinfo(:,2);
    else
...
ud{6} = getappdata(gcf,'SUBQUERYDATA');
ud{7} = getappdata(gcf,'SUBQUERYSTRING');
ud{8} = getappdata(gcf,'TABLEVALUES');
ud{9} = getappdata(gcf,'FIELDVALUES');
...
function applyordergroup(obj,evd,frame) %#ok
...
function applysubquerydlg(obj,evd,frame)   %#ok
...
if isempty(fdat{1})
  return   %Do not build query if no fields are selected
end
fstr = fdat{1}(fdat{2});
...
function catalogs(obj,evd,frame)
%CATALOGS Catalog selection

ui = getappdata(frame,'uidata');

if isempty(get(ui.sources,'String'))
  %No defined datasource file 
  return
end
...
if strcmp(class(conn.Handle),'double')
  ...
  set(getappdata(0,'SQLDLG'),'Pointer','arrow')
  return    %Invalid connection, stop processing
end

%Get selected schema (only one schema at a time is supported)
catval = get(ui.catalogs,'Value');
schval = get(ui.schemas,'Value');

%Deselecting all schemas shows default table list for database
if catval == 1 && schval == 1
  sources(obj,evd,frame)
  return
end
set(ui.catalogs,'Value',catval)
catstr = get(ui.catalogs,'String');
catalog = catstr{catval};
...
schval = get(ui.schemas,'Value');
if schval ~= 1
  schstr = get(ui.schemas,'String');
  schema = schstr{schval};
  schemastr = [schema '.'];
  catalogstr = [catalog '.'];
else
  schema = {};
...
%Trap <default> setting for catalog
if catval == 1
  catalog = [];
  catalogstr = [];
end

%Set product name for Database specific table name search
if ~isempty(schema) || ~strcmp(catalog,'<default>')
  p = '   ';
else
...
%Get tables
try
  set(ui.tables,'String',[],'Value',[])
  t = alttables(conn,catalog,p,d,schema);
catch exception
...
%Add catalog and schema to table name
for i = 1:size(t,1)
  t{i,1} = [catalogstr schemastr t{i,1}];
end
...
%Set table list
set(ui.tables,'String',t(:,1),'Value',[],'Listboxtop',1)
%Selecting a new catalog erases any clause data
...
%Clear field list
set(ui.fields,'String',[],'Value',[],'Listboxtop',1)
%Clear SQL statement
set(ui.query,'String',[])

function clausedialog(tagname,bspc,bwid,bhgt)
...
%Build dialog frames
uipanel('Units','pixels','Bordertype','beveledout','Position',[bspc 4*bspc+5*bhgt 5*bspc+7*bwid 6*bhgt]);
...
uipanel('Units','pixels','Bordertype','beveledout','Position',[bspc bspc 5*bspc+7*bwid bspc+5*bhgt]);
...
function definejdbc(obj,evd,frame)  %#ok
%DEFINEJDBC Call JDBC data source configuration dialog

confds


function defineodbc(obj,evd,frame)  %#ok
%DEFINEODBC Call system ODBC data source configuration manager.

s = system('%SystemRoot%\system32\odbcad32.exe');
if s
  errordlg('Unable to run ODBC data source administrator.')
  return
end


function deleteclause(obj,evd,frame)  %#ok
...
fldobj = findobj(gcf,'Tag',[tagname 'fields']);
if length(get(fldobj,'String')) < wcdat{wcval,1}
  set(fldobj,'Value',1)
else
...
if any(strcmp(get(findobj(uicond.rcond,'Value',1),'String'),{'Relation','In'}))
  set(uicond.subquery,'Enable','on')
  setappdata(gcf,'SUBQUERYDATA',wcdat{wcval,6})
  setappdata(gcf,'SUBQUERYSTRING',wcdat{wcval,7})
  setappdata(gcf,'TABLEVALUES',wcdat{wcval,8})
  setappdata(gcf,'FIELDVALUES',wcdat{wcval,9})
else
...
try
  ...
  errordlg('Please choose a datasource.')
  set(findobj('Type','figure'),'Pointer','arrow')
  return
end
  ...
  case 'Select'
    ...
    if strcmp(dfmt,'structure')
      ...
      while 1  
        ...
        if ~isa(curs.Data,'struct') & ~strcmp(curs.Data,'No Data') %#ok
          ansf = [ansf;curs.Data];   %#ok
        else
          break
        end
      end
...
function genmfile(obj,evd,frame)
%GENMFILE MATLAB file code from current dialog settings.
...
%setdbprefs MATLAB code
p = setdbprefs;
f = fieldnames(p);
mstr = ['% Set preferences with setdbprefs.',sprintf('\n')];
for i = 1:length(f)
  mstr = [mstr,'s.' f{i} ' = ''' p.(f{i}) ''';',sprintf('\n')];
end
mstr = [mstr,'setdbprefs(s)',sprintf('\n\n')];

%Connection call MATLAB code
mstr = [mstr,'% Make connection to database.  Note that the password has been omitted.',sprintf('\n')];
conn = loginconnect(sourcestr);
close(conn)
if isempty(conn.URL)
  mstr = [mstr,'% Using ODBC driver.',sprintf('\n')];
  mstr = [mstr,'conn = database(''' sourcestr ''',''' conn.Username ''',''password'');',sprintf('\n\n')];
else
  mstr = [mstr,'% Using JDBC driver.',sprintf('\n')];
  mstr = [mstr,'conn = database(''' sourcestr ''',''' conn.Username ''',''password'',''' conn.Driver ''',''' conn.URL ''');',sprintf('\n\n')];
end

%Read/write MATLAB code
switch rwstr
    case 'Select'
      %Trap single quotes
      mstr = [mstr,'% Read data from database.',sprintf('\n')];
      querystr = strrep(querystr,'''','''''');
      mstr = [mstr,...
        'e = exec(conn,''' querystr ''');',sprintf('\n'),...
        'e = fetch(e);',sprintf('\n'),...
        'close(e)',sprintf('\n\n')];
    
      %Put data in specified output variable
      if ~isempty(wkvar)
        mstr = [mstr,'% Assign data to output variable.',sprintf('\n')];
        mstr = [mstr,wkvar ' = e.Data;',sprintf('\n\n')];
      end
    case 'Insert'
      mstr = [mstr,'% Write data to database.',sprintf('\n')];
      mstr = [mstr,querystr,sprintf('\n\n')];
end

%Close connection MATLAB code
mstr = [mstr,'% Close database connection.',sprintf('\n')];
mstr = [mstr,'close(conn)',sprintf('\n')];

%Send MATLAB code to editor
matlab.desktop.editor.newDocument(mstr);

set(findobj('Type','figure'),'Pointer','arrow')


function getjdbcfile(obj,evd,frame,bname)
%GETJDBCFILE JDBC data source file location.

ui = getappdata(frame,'uidata');

%Specify new or point to existing file
switch bname
    case 'new'
    ...
    case 'def'
      jdbcfile = get(ui.jdbcdatasourcefile,'String');
      if isempty(jdbcfile)
        set(ui.define,'Enable','off')
      else
        set(ui.define,'Enable','on')
      end
...
ui = getappdata(frame,'uidata');
[~,~,bspc,bhgt,bwid] = spaceparams(obj,evd,frame);
...
if ~isempty(wval)
  ...
  i = strfind(wstr{wval(nvals)},' AND');
  if isempty(i)
    i = strfind(wstr{wval(nvals)},' OR');
  end
  if isempty(i)
...
try  
  ...
  %Need to reset catalog and schema information if used
  %Account for older versions of .qry files 
  if exist('catdat','var')
    set(ui.catalogs,'Value',catdat)
    catalogs(obj,evd,frame)
    set(ui.schemas,'Value',schdat)
    schemas(obj,evd,frame)
  end
  ...
  if exist('querystr','var')
    if strcmpi(querystr(1:6),'INSERT')
...
function rrcond(obj,evd,frame)   %#ok
...
function savevqb(obj,evd,frame)  %#ok
...
%Get source, catalog, schema, table and field information for saving
...
%Get catalog and schema settings
catdat = get(ui.catalogs,'Value');
schdat = get(ui.schemas,'Value');
...
try 
  eval(['save ''' p f ''' sstr catdat schdat tstr fstr aodval wdat gdat hdat odat querystr -mat'])
...
function schemas(obj,evd,frame)
%SCHEMAS Schema selection
...
%Set product name for Database specific table name search
if ~isempty(catalog) || ~strcmp(schema,'<default>')
  p = '   ';
else
  p = get(d,'DatabaseProductName');
end
...
for i = 1:size(t,1)
  t{i,1} = [catalogstr schema{1} '.' t{i,1}];
end
...
%Reset fields listbox, query, workspace variable and data string so
%incorrect fieldnames are not showing for chosen source
set([ui.fields,ui.query,ui.wkvariable],'String',[])
set(ui.catalogs,'String',[{'<default>'};cats],'Value',1)
set(ui.schemas,'String',[{'<default>'};sch(:)],'Value',1)
set(ui.tables,'String',[],'Value',[],'Listboxtop',1)

try
  drf = setdbprefs('DataReturnFormat');
  setdbprefs('DataReturnFormat','cellarray')
  [tableName,tableType] = alttables(conn,{},p,d,{});
...
%Set table display
set(ui.tables,'String',table(:),'Value',[],'Listboxtop',1)
...
try
  ...
  %Not enough information to build query, do nothing
end
...
function subqfields(obj,evd,frame)   %#ok
...
uipanel('Units','pixels','Bordertype','beveledout','Position',[bspc bspc fwid fhgt1]);
...
uipanel('Units','pixels','Bordertype','beveledout','Position',[bspc 3*bspc+fhgt1 fwid fhgt2]);
uipanel('Units','pixels','Bordertype','beveledout',...
...
uipanel('Units','pixels','Bordertype','beveledout','Position',[bspc 5*bspc+fhgt1+fhgt2 fwid bspc+4*bhgt]);
...
sqdat = getappdata(findobj('Tag',dlgtag),'SUBQUERYDATA');
sqstr = getappdata(findobj('Tag',dlgtag),'SUBQUERYSTRING');
tval = getappdata(findobj('Tag',dlgtag),'TABLEVALUES');
fval = getappdata(findobj('Tag',dlgtag),'FIELDVALUES');
...
for i = 1:L
  ...
  switch sDbName  %Query only a single row where possible for speed
    ...
    case {'MySQL'}
      ex = exec(conn,['select * from ' table ' LIMIT 0']);
    case {'Microsoft SQL Server','ACCESS'}
      ex = exec(conn,['select TOP 1 * from ' table]);
    case {'Oracle'}
      ex = exec(conn,['select * from ' table ' where ROWNUM <= 1']);  
    otherwise
      ex = exec(conn,['select * from ' table]);
  ...
  ex = fetch(ex,1);
  atrib = attr(ex);
  tabfields = cellstr(char(atrib.fieldName));
  close(ex); 
...
%Display fieldnames
set(ui.fields,'String',fields,'Value',[],'Listboxtop',1);
...
function varlist(obj,evd,frame) %#ok
