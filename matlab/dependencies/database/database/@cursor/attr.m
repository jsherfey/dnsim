...
if (nargin == 1)
  ...
  if (isa(cursor,'cursor') ~= 1)
     ...
     attributes.Message = 'First argument of function must be a cursor';
     return;
...
elseif (nargin == 2)
  ...
  if ((isa(cursor,'cursor') ~= 1) | ...
   ...
   attributes.Message = 'Arguments of function must be a cursor and column value';  
   return;
...
if (nargin == 1)
   ...
   for (i = 1:double(columns))
     ...
     % Get the precision of the field if applicable ... test for floats,reals,doubles etc. 
  ...
  if ( (attributes.typeValue == 2)      | ...
    ...
    attributes.precision = precision(cursor.Fetch,i,resultSetMetaData);
  else,
