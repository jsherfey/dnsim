...
MainFig = figure('tag',FigTag,'Visible','off','WindowButtonMotionFcn',@ShowValue,'WindowScrollWheelFcn',@ZoomFunction);
...
%             allchannels(n).allsets{j} = dat;
      ...
      if get(gethandle('eventchk'),'value') && ~isempty(options.events(1).label) && ismember(this,{options.events.label})
        ...
        for k = 1:length(typ)
          % modified on 11-Feb-2013
          color = colors(mod(typ(k)-1,length(colors))+1);
          tmp = evt([options.events(chix).type(tix)]==typ(k));
          for j = 1:length(tmp)
            vline(tmp(j),'Color',color,'ButtonDownFcn',@onclick);
          end
        end
        clear tmp
      end
